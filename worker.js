...
- 不明なものは不明のままにする。
- 「重要」「大事」と言われた場合、緊急性・将来必要・価値がある等のどの意味か必要に応じて確認する。
...
try {
  state = JSON.parse(localStorage.getItem("carebot_v2") || "null") || initial;
} catch(e) {
  state = initial;
}
...
function save(){
  localStorage.setItem("carebot_v2",JSON.stringify(state));
}
...
function home(){
  ...
  set(
    ...
    '<div class="row">'+
      ...
      '<button id="checkBtn">確認する</button>'+
    '</div>'+
    ...
    '<div class="card">'+
      ...
      '<button class="primary" id="sendBtn" style="margin-top:10px">送る</button>'+
    '</div>'+

    '<div class="card">'+
      '<b>今確認すること</b>'+
      '<div class="listItem">母の退院日 <span class="badge">確認待ち</span></div>'+
      '<div class="listItem">9月16日 9:00 竜操整形外科病院</div>'+
    '</div>'
  );

  document.getElementById("recordBtn").onclick = recordStart;
  document.getElementById("checkBtn").onclick = checkMenu;
...
function recordStart(){
  ...
  set(
    ...
    '<button class="primary" id="recordNext" style="margin-top:10px">次へ</button>'
  );

  document.getElementById("recordNext").onclick = function(){

    var t = document.getElementById("recordText").value.trim();
...
function checkMenu(){
  ...
  set(
    ...
    '<div class="row">'+
      '<button id="showAppts">予定・予約</button>'+
      '<button id="showTodos">やること</button>'+
      '<button id="askText">自分の言葉で聞く</button>'+
    '</div>'
  );

  document.getElementById("showAppts").onclick = showAppointments;
  document.getElementById("showTodos").onclick = showTodos;

  document.getElementById("askText").onclick = function(){

    set(
      '<h2>何を知りたいですか？</h2>'+
      '<textarea id="q"></textarea>'+
      '<button class="primary" id="askGo" style="margin-top:10px">確認する</button>'
    );
...
function submitFree(isQuestionMode){

  var el = document.getElementById(
    isQuestionMode ? "q" : "free"
  );
  ...
  interpretThenRoute(text,isQuestionMode);
}

function interpretThenRoute(text,isQuestionMode){

  set(
    '<div class="notice">AIが確認しています…</div>'
  );
...
function routeAI(text,ai,isQuestionMode){
  ...
  var eventVerb =
    /行く|受診|予約|通院|検査|入院|退院|面談|訪問|来る/.test(text);

  if(!explicitQuestion && hasDate && eventVerb){
...
function hospitalFromText(text){
...
function showAppointmentConfirm(text,ai){
  ...
  set(
    ...
    '<div class="row">'+
      '<button class="primary" id="saveAppt">記録する</button>'+
      '<button id="editAppt">修正する</button>'+
    '</div>'
  );
  ...
  document.getElementById("editAppt").onclick =
    editAppointment;
}
...
function editAppointment(){
  ...
  set(
    ...
    '<div class="label">時刻（分からなければ空欄）</div>'+
    ...
    '<button class="primary" id="editDone" style="margin-top:10px">確認へ</button>'
  );
...
function saveAppointment(){
  ...
  set(
    ...
    '<b>予定を記録しました。</b>'+
    '</div>'+
    '<button id="seeAppts" style="margin-top:10px">予定一覧を見る</button>'
  );
...
function showGeneralConfirm(text,ai){
